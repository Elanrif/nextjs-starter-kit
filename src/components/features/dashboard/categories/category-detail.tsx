...
export function CategoryDetail({ data }: { data: Category | null }) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5">
      {/* Hero */}
      <div className="relative overflow-hidden rounded-2xl bg-linear-to-br from-slate-900 via-violet-950 to-slate-900 shadow-xl">
        <div className="pointer-events-none absolute -top-24 -right-24 h-80 w-80 rounded-full bg-violet-500/20 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-16 -left-12 h-56 w-56 rounded-full bg-purple-500/15 blur-3xl" />

        <div className="relative p-8">
          <div className="flex items-start gap-6">
            {/* Icon */}
            <div className="relative shrink-0">
              <div className="h-20 w-20 rounded-2xl bg-violet-500/20 ring-2 ring-violet-400/30 flex items-center justify-center shadow-lg">
                <Tag className="w-9 h-9 text-violet-300" />
              </div>
              <div
                className={`absolute -bottom-2 -right-2 w-5 h-5 rounded-full border-2 border-slate-900 ${
                  category.isActive ? "bg-emerald-400" : "bg-red-400"
                }`}
              />
            </div>
