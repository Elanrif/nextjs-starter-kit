...
const ic =
  "w-full pl-10 pr-4 py-2.5 rounded-xl border border-white/10 bg-white/5 text-sm text-white placeholder-white/25 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500/50 transition-all disabled:opacity-50";
const icPwd =
  "w-full pl-10 pr-10 py-2.5 rounded-xl border border-white/10 bg-white/5 text-sm text-white placeholder-white/25 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500/50 transition-all disabled:opacity-50";
...
export function SignUpForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      {password && (
        <div
          className={`p-2.5 rounded-xl space-y-1 border ${allValid ? "bg-emerald-500/10 border-emerald-500/20" : "bg-white/5 border-white/8"}`}
        >
          <div className="flex items-center gap-1.5 mb-1">
            <ShieldCheck
              className={`w-3 h-3 ${allValid ? "text-emerald-400" : "text-white/30"}`}
            />
            <span
              className={`text-[10px] font-semibold ${allValid ? "text-emerald-400" : "text-white/40"}`}
            >
      ...
      <button
        type="button"
        onClick={fillDemo}
        className="w-full flex items-center justify-center gap-2 py-2 rounded-xl border border-white/10 bg-white/5 hover:bg-white/8 text-white/50 hover:text-white/70 text-xs transition-colors"
      >
